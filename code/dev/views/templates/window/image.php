<div class="image-crop-container">
    <div class="group-container">
        <div class="right-column">
            <div class="preview-label"></div>
            <div class="preview"></div>

            <div class="actions">
                <div class="actions-label"></div>
                <div class="action-container rotate-container btn-group"></div>
                <div class="action-container flip-container btn-group"></div>
                <div class="action-container zoom-container btn-group"></div>
                <div class="action-container user-aspect-container btn-group"></div>
                <div class="action-container default-aspect-container btn-group"></div>
                <div class="action-container reset-container"></div>
            </div>
        </div>

        <div class="left-column">
            <div class="crop-label"></div>

            <div class="canvas">
                <img class="cropper" src="" />
            </div>
        </div>
    </div>
</div>

<div class="image-group-sort-container"></div>

<div class="image-group-sort-item">
    <img src="" />
    <div class="buttons">
        <div class="background"></div>
    </div>
</div>